...
for entry in range(0, numberOfEntries):
  ...
  for igen,gen in enumerate(branchParticle):
    ...
    if(abs(gen.PID) == 1000006):
      # and gen.Mass >= 500 and gen.Mass <= 1000):
      STOP_idx = igen
    ...
    #print("comin after LSP and STOP idx selection")
      #    if (igen == STOP_idx or igen == LSP_idx): continue
    if(not(STOP_idx >= 0 and LSP_idx>= 0)): continue
    #print("comin inside if idx stp and lsp")
    STOP = branchParticle.At(STOP_idx)
    LSP = branchParticle.At(LSP_idx)


    #print("STOP Mass", STOP.Mass)
    #print("LSP Mass", LSP.Mass)
    if(not(STOP.Mass == 1000 and LSP.Mass == 100)): continue
    print("comin in loop")
    count_1 += 1
